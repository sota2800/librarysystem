...
# 2. システムの内容
## 2.1 目的
対話型インタフェースの情報サービスにおいて，音声対話を通してユーザの要求を抽出し，Webブラウザのスクレイピングにより情報サービスシステムの操作および情報抽出を行うことで，ロボットによるサービス提供機能を追加するための枠組みを提案する．
作例として，現在大学図書館で稼働しているヘルプデスク支援ロボットのRTコンポーネント化を行う．
## 2.2 内容
本システムは対話型インタフェースの情報サービスシステムにおいて，以下の機能を提供し，ロボットによるサービスを既存システムの改良をせずに提供できる．
- 音声対話によるユーザからの要求の抽出
- Webブラウザのスクレイピングによる情報サービスシステムの操作および情報抽出
## 2.3 特徴
- ロボットシステムが人間向けのソフトウェアを直接利用可能：既存システムのAPIやSDKを利用したプログラミングが不要であり，短期間での開発を実現.
- 対話機能はクラウドAIサービスを利用→内製あるいはインストールする必要がなく，AIの発展の恩恵をリアルタイムで受けられる．
# 3. 開発環境
本システムの開発環境を以下に記す．
